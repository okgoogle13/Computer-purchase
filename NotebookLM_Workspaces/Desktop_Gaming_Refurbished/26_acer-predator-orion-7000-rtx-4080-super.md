<!-- TAGS: #DesktopTower #NVIDIA #VRAM-16GB #Refurbished #SingleGPU #PrimaryWorkstation #NeedsReview -->
---
id: acer-predator-orion-7000-rtx-4080-super
category: desktop
name: Acer Predator Orion 7000 — RTX 4080 Super (Refurbished)
gpu: NVIDIA GeForce RTX 4080 (16 GB)
vram: 16 GB
score: 82
track: track_1.5
category: refurbished_gaming_desktop
price_performance_vs_track1: CALCULATE (desktop_price / laptop_comparable_price)
vram_advantage_vs_track1: CALCULATE (desktop_vram - laptop_vram_gpu_available)
portability_penalty: true
---

# Acer Predator Orion 7000 — RTX 4080 Super (Refurbished)

## Overview
- **VRAM Tier:** Priority 3 — 16 GB
- **Adjusted Score:** 82 / 110
- **Price (AUD):** ~$3,699
- **Vendor:** Acer AU Clearance

## Key Specs
- **GPU:** NVIDIA GeForce RTX 4080 16 GB (Ada Lovelace)
- **CPU:** Intel Core i9-14900K
- **RAM:** 32 GB DDR5
- **Storage:** 1 TB SSD + 2 TB HDD
- **Form factor:** OEM Gaming tower

## AI Capability Summary
Strong i9 + RTX 4080 combination in a refurbished chassis. Good for 7B–14B models. The OEM Acer case and mail-in warranty are limitations. Loud under sustained AI load.
